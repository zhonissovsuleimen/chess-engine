...
impl Board {
  ...
  pub fn move_piece(&mut self, from_id: usize, to_id: usize) -> bool {
    ...
    if self.is_pawn(from_mask) {
      new_moves_mask = self.gen_pawn_move(from_mask)
        | self.gen_pawn_advence_move(from_mask)
        | self.gen_pawn_capturing_moves(from_mask);
    }
  ...
  pub fn gen_knight_moves(&self, at_mask: u64) -> u64 {
    ...
    king_moves += at_mask.checked_shr(9).unwrap_or(0);
    king_moves += at_mask.checked_shr(8).unwrap_or(0);
    king_moves += at_mask.checked_shr(7).unwrap_or(0);
    king_moves += at_mask.checked_shr(1).unwrap_or(0);
    king_moves += at_mask.checked_shl(1).unwrap_or(0);
    king_moves += at_mask.checked_shl(7).unwrap_or(0);
    king_moves += at_mask.checked_shl(8).unwrap_or(0);
    king_moves += at_mask.checked_shl(9).unwrap_or(0);
